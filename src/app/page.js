...
const SPORT_FACTOR_BLOCKS = {
  ...
  nfl: `NFL: TRENCHES — O-line vs D-line matchup (PFF grades), pass-rush pressure rate created and allowed. QB MATCHUP (its own signal, not just a team-efficiency number) — the question is whether this QB can function against what THIS defense specifically does: pressure-rate performance, turnover-worthy-play rate, mobility, arm strength/accuracy, and how that holds up against this defense's coverage and pass-rush scheme, not just a raw stat line. OFFENSIVE/DEFENSIVE EFFICIENCY — pass and rush efficiency both directions, third down conversion rate, red zone TD% vs FG%, two-minute drill efficiency, OC/DC tendencies. KEY INJURIES — official Wed/Thu/Fri practice designations (Full/Limited/DNP). A "Questionable" tag is a resolved read, not a gap — weigh it on the merits (a Questionable starting QB or a pass rusher central to the game plan is a real risk; a Questionable depth piece usually isn't). Only a genuine failed lookup on a player who actually matters to this specific bet is the real unknown — if that happens, say so plainly and give your read both ways rather than guessing or refusing. SPREADS — key-number awareness is critical: 3, 7, and 10 are the core NFL margins, and the half-point hooks (3.5, 7.5, 10.5) matter — note when a line sits on, just off, or has moved through one of these. TOTALS — weather (wind 15mph+ kills passing and favors unders) and pace/game-script fit are critical factors, not background color. Stadium noise factor (road team silent counts). AVOID THE NARRATIVE TRAP — rivalry, "revenge game," and motivation storylines are not a real signal; sportswriting can be found to support almost any story. Note the verifiable facts (division game, playoff stakes, recent head-to-head history) and let those inform the read, not a claim about who wants it more. WEEK 1 / EARLY SEASON — "last 3 games" doesn't exist yet; say so, lean on returning starters, scheme continuity, and preseason reporting instead, and hold early-season reads to a more conservative confidence than mid-season ones.`,
  ...
  ncaaf: `College Football (NCAAF): Same baseline as NFL (trenches, QB matchup framed against this specific defense, offensive/defensive efficiency, key-number awareness for spreads, weather/pace for totals) plus recruiting talent gap (blue chip ratio), home field crowd advantage (especially top-10 atmospheres), conference vs non-conference performance, transfer portal impact on depth. INJURY RELIABILITY SPLITS BY CONFERENCE — treat this as real, not a technicality: as of 2025 the four Power conferences (SEC, Big Ten, Big 12, ACC) mandate enforced injury reports (Wednesday start, Friday/near-kickoff updates), so Power 4 injury data is close to NFL-reliable. Group of 5 (AAC, C-USA, MAC, Mountain West, Sun Belt) and independents have no such mandate and reporting is inconsistent — that's a real data gap, not a reason to skip these games; G5 and small-conference lines are often the softest and most mispriced on the board. What matters is whether the SPECIFIC player your bet leans on can be confirmed: a Power 4 game with no news on a bench piece is fine, but a G5 game where you can't confirm the starting QB or another player central to your thesis is the real case — say plainly you can't confirm it, give your read both ways, and name that as the swing factor rather than pretending to a confidence you don't have. AVOID THE NARRATIVE TRAP — rivalry is a fact (yes or no, plus real series history); motivation is not a stat. Note when a game genuinely is a rivalry or a big non-conference matchup, but don't let a "team X wants this one more" story substitute for the structural factors above doing the actual work. WEEK 1 / EARLY SEASON — even more pronounced than NFL, since rosters turn over heavily through the transfer portal; last year's tape is a weaker guide here, so lean on returning production and portal additions you can actually confirm joined, and hold early-season and small-sample reads to conservative confidence.`,
